import type { KnowledgeArea } from '../types';

export const kaGAME: KnowledgeArea = {
    id: "ka-game-dev",
    title: "Game Development",
    description: "Unity, Unreal, design de jogos.",
    load: "40h",
    iconName: "Gamepad2",
    theory: [{
        id: "game-t1",
        title: "Criando Mundos Interativos",
        content: `
            <h2>O que é um Game Engine?</h2>
            <p>Um game engine (motor de jogo) é um framework de software projetado para a criação e desenvolvimento de jogos eletrônicos. Ele fornece funcionalidades essenciais como renderização de gráficos, motor de física, detecção de colisão, áudio, etc.</p>
            <h3>Engines Populares:</h3>
            <ul>
                <li><strong>Unity:</strong> Popular para jogos mobile e indie, usa a linguagem C#.</li>
                <li><strong>Unreal Engine:</strong> Conhecido por gráficos de alta fidelidade, usa C++ e um sistema de scripting visual chamado Blueprints.</li>
            </ul>
        `
    }],
    practice: {},
    quizzes: []
};
