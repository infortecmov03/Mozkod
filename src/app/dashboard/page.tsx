...
export default function DashboardPage() {
  ...
  const getModuleProgress = (module: typeof modules[0]) => {
    ...
    module.knowledgeAreas.forEach(ka => {
      ...
      completed += ka.theoryLessons.filter(l => isCompleted(l.id)).length;
      completed += ka.practiceExercises.filter(p => isCompleted(p.id)).length;
    });
          ...
          <Card className="bg-card/40 border-none shadow-xl group hover:bg-card/60 transition-all cursor-pointer p-6">
             <div className="flex items-center gap-4">
               ...
               <div className="hidden sm:block">
                 ...
                 <p className="text-xl font-headline font-bold">{progress?.length || 0}</p>
               </div>
        ...
        <section className="space-y-8">
          ...
          <div className="grid gap-6">
            {modules.map((module) => {
              ...
              const imgUrl = getImg(module.image);
