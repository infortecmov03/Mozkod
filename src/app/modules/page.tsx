...
import { ArrowRight, BookOpen, Code2, CheckCircle2, Circle, GraduationCap, ChevronRight } from "lucide-react";
...
export default function ModulesPage() {
  ...
  const getModuleProgress = (module: any) => {
    ...
    module.knowledgeAreas.forEach((ka: any) => {
      total += ka.theoryLessons.length + ka.practiceExercises.length;
      completed += ka.theoryLessons.filter((l: any) => isCompleted(l.id)).length;
      completed += ka.practiceExercises.filter((p: any) => isCompleted(p.id)).length;
    });
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                           ...
                           <Card className="bg-background/40 border-white/5 hover:border-primary/20 transition-all overflow-hidden group/card shadow-xl">
                              <CardContent className="p-6 space-y-6">
                                ...
                                <Link href={`/learn/${ka.theoryLessons[0].id}`}>
                           ...
                           {ka.practiceExercises.length > 0 ? (
                             <Card className="bg-background/40 border-white/5 hover:border-accent/20 transition-all overflow-hidden group/card shadow-xl">
                               <CardContent className="p-6 space-y-6">
                                  ...
                                  <div className="space-y-2">
                                     {ka.practiceExercises.map(ex => {
                                       const theoryId = ka.theoryLessons[ka.practiceExercises.indexOf(ex)]?.id;
                                  ...
                                  <Link href={`/learn/${ka.practiceExercises[0].id}`}>
